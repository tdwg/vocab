...
DCMI doesn't specify any particular process for accumulating use cases or implementation reports.  It does assume that the needs of particular communities will be considered and that application profiles will be developed by a team with specialized knowledge of the resources that need to be described.
...
Because the Vocabulary Management Specification that we are developing relates specifically to vocabularies and not necessarily to the development of processing applications, the examples listed for SKOS and PROV are probably the most relevant.  In other words, compiling use cases early in the process in order to determine the important requirements of the vocabulary enhancement is probably important.  Reporting on implementation experience is most likely to include reporting on which terms and features were used as parts of vocabularies or to represent datasets, rather than test suites to determine the features that were successfully implemented in developed applications.  However, there may be cases where running particular tests might be important.  It probably should be left to the developing group to determine whether such cases exist for a particular vocabulary enhancement.
